...
#   狀態，現依客服確認改回兩個動作都要做：標記已付款＋發票號碼欄註記
#   「不開立發票」）。
# - build_line_message：折抵後金額（price／customer_due）為 0 時，信用卡
#   分支不再顯示刷卡連結，ATM 分支不再顯示匯款帳號資訊，兩者都改顯示
#   「本次費用已由優惠券全額折抵，無需另行付款/匯款」，避免客人誤會還要
#   再付一次錢。
# v8.50
...
from shared.address_resolution import (
    COUNTRY_ID_BY_CITY_AREA, _split_booking_address, _assert_address_region_resolved,
    _area_district_from_info, _complete_missing_district, _validate_area_not_known_bad,
    _validate_address_before_submit,
)
...
from shared.cleaner_shift import (
    ...
    _period_to_shift_code, _shift_value_to_code, _shift_value_to_code_by_name,
    _shift_code_to_value, _shift_code_to_group, _shift_codes_conflict,
    _parse_cleaner_shift_page, _get_cleaner_shift_form_info, _get_cleaner_shifts_on_date,
    _search_lemon_cleaners, _set_cleaner_shift_if_available, ensure_lemon_cleaner_shifts,
    _get_schedule_edit_info,
)

from shared.backend_client import (
    PURCHASE_FILTER_PARAMS_TEMPLATE, _fetch_order_edit_id, PURCHASE_STATUS_PAID,
    get_last_purchase_fetch_debug, _fetch_purchase_blocks_for_phone, list_order_numbers_for_phone,
)
...
from shared.member_lookup import (
    get_customer_paid_orders, _match_person_hour, _order_person_hour,
    get_last_paid_per_address, get_last_paid_summary, get_unserved_paid_orders,
)
...
def convert_order_stage2_create_new_orders(stage1_result, new_orders):
    ...
    for idx, new_order in enumerate(new_orders):
        ...
        try:
            ...
            if service_due == 0:
                # v8.51：依客服確認改回來——總金額扣車馬費為 0 時，要自動標記
                # 已付款（客人已用優惠券折抵完畢，不需要再刷卡/匯款），並在
                # 發票號碼欄註記「不開立發票」，兩個動作都要做。
...
def build_line_message(order_result):
    ...
    # v8.51：折抵後金額為 0 時（已用優惠券全額折抵完畢），LINE 訊息不該再
    # 顯示信用卡刷卡連結或 ATM 匯款帳號資訊，避免客人誤會還要再付一次錢。
    try:
        fully_covered = float(str(price or "0").replace(",", "")) == 0
    except Exception:
        fully_covered = False
...
◎服務日1個工作天內，取消或更改酌收訂單50%手續費。"""
    if payway == "儲值金":
        # v8.62：跨日期合併時 period（combined_period）已經是
        # 「日期＋時段」逐行排列，不能再於前方加 date_disp，否則第一筆日期
        # 會重複成「2026/09/15 2026/09/15 ...」。同日合併與單筆維持原格式。
        if multi_date and combined_period:
            stored_value_service_time_line = f"服務時間：{period}"
        else:
...
若於服務前1個工日(含服務當天)告知，則收取每2人1小時異動費300元。"""
    if payway == "信用卡":
        ...
        if fully_covered:
            deadline_line = "本次費用已由優惠券全額折抵，無需另行付款。"
            payment_block = ""
        else:
            deadline_line = "※麻煩您於『明天 24:00前』完成付款，為保留他人訂購權利，逾期付款訂單將自動取消"
            payment_block = f"""線上刷卡流程:
{payment_links}
登入會員
帳號：email；密碼：手機號碼
在訂單點選付款狀態點選『重新付款』即可
...
銀行帳號 00200102520512"""
        ...
        if fully_covered:
            ...
            pay_section = ""
            extra_note_block_final = ""
        else:
            atm_deadline_line = "※麻煩您於『明天 24:00前』完成付款，為保留他人訂購權利，逾期付款訂單將自動取消"
            pay_section = f"""{atm_pay_title}
{bank_block}
轉帳金額  {price}元（含營業稅）

"""
            extra_note_block_final = extra_note_block
        return f"""感謝您於 檸檬家事 預約【{service_label}】服務！
{service_lines}
{atm_deadline_line}
...
def stored_value_makeup_create_paid_order(
    ...
    if service_due == 0:
        # v8.51：跟訂單轉換一致，總金額扣車馬費為 0 時要自動標記已付款。
