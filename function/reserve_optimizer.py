...
# 功能：檸檬保留單班表分析、保留率計算、即時未配班檢查與批次建立保留訂單。
# 更新時間：2026-08-19
# ============================================================
# -*- coding: utf-8 -*-
"""檸檬保留單：班表分析與批次建單。從 orders-system 移植到模組化 order-system。"""
...
def calculate_reserve_target(unassigned_people, reserve_rate):
    ...
    reserve_orders = int(math.floor((people * normalize_rate(reserve_rate)) / 2.0))
    reserve_people = reserve_orders * 2
    return reserve_people, reserve_orders, max(0, people - reserve_people)
...
def snapshot_day(lookup_result, service_date, rules, periods):
    ...
    for period in periods:
        ...
        rp, ro, mp = calculate_reserve_target(people, rate)
...
def create_reserve_orders_for_plan(*, env_name, lookup_result, region, address, plan_rows, payway, clean_type_id="1", continue_after_slot_error=True, sleep_seconds=0.5):
    ...
    for row in plan_rows or []:
        ...
        if qty <= 0:
            continue
        target += qty
        date_s, period = str(row.get("service_date") or ""), str(row.get("period") or "")
        for seq in range(1, qty + 1):
            try:
                current = parse_schedule_unassigned(fetch_schedule_html(lookup_result, date_s), date_s)
                people = int((current.get(period) or {}).get("unassigned_people") or 0)
                if people < 2:
                ...
                flat.append({
                    "success": True, "order_no": result.get("order_no"), "staff": result.get("staff"),
                    "service_date": date_s, "period": period, "batch_id": batch_id,
                    "customer_memo_ok": memo_ok, "customer_memo_message": memo_msg,
                    "note_ok": note_ok, "note_message": note_msg,
                })
                if not memo_ok:
                    break
            except Exception as exc:
